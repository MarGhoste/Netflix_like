<div class="min-h-screen bg-black text-white">
    ...
    <!-- Menú lateral (ASIDE) que ya está factorizado y se carga automáticamente -->
    <x-app.sidebar />
    ...
    <main class="ml-20">
        ...
        <div class="relative w-full h-96 bg-gray-800 rounded-lg overflow-hidden shadow-2xl">
            ...
            @if ($heroMovie)
                ...
                <div
                    ...
                    <h2 class="text-5xl font-extrabold mb-3 text-white">
                        {{ $heroMovie->title }}</h2>
        ...
        {{-- -------------------------------------------------------------------------------- --}}
        {{-- === [NUEVO] CARRUSELES DINÁMICOS POR GÉNERO === --}}
        @if (isset($genresWithMovies) && $genresWithMovies->count() > 0)
            <h3 class="text-2xl font-bold mt-12 mb-4">Explorar por Género</h3>
            @foreach ($genresWithMovies as $genre)
                {{-- Usamos el componente que ya existe, pasando el nombre del género como título --}}
                {{-- La propiedad $genre->movies se carga gracias a la relación 'with' en el componente Livewire --}}
                @if ($genre->movies->count() > 0)
                    <x-movie-row-carousel title="{{ $genre->name }}" :movies="$genre->movies" />
                @endif
            @endforeach
        @endif
        {{-- -------------------------------------------------------------------------------- --}}
